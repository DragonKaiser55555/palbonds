...
## Passes 232-241 (2026-09-07): following made usable, then made correct

Everything from the pass-231 breakthrough to a state Dragón confirmed with "excelent, it worked".

**232 — the re-assert got its own fast loop.** Piggybacking on the 1500ms follower tick meant the action won the race most of the time and following came in bursts. Now a dedicated 100ms loop, a cached player actor (never a world lookup), and a 1000ms idle backoff so the normal case — nobody bonding — costs one table lookup a second. Measured result: `Trainer` was found already cleared **once in 1101 checks**. It is wiped once at startup, not continuously.

**233 — the follower's position.** Dragón: *"otomo pals following is impossible to chase... they always try to stay on screen where the player is looking and always at a distance, that makes approaching them impossible"*. This is a real blocker rather than a nuisance, because the whole interaction layer is aim-based: a follower that keeps itself out of reach cannot be petted, fed or played with, so making following work perfectly would have made the mod unusable. The field dump already held the answer, on every Pal: `TargetLocationDistanceForward = 800`, `TargetLocationDistanceRight = 300` — the Pal steers to a point eight metres AHEAD of the player. Set to -220 with per-follower lateral slots (0, ±260, ±520). No funnel class needed; `BP_AIAction_FunnelFollow_C` differs only in these same numbers. The lateral spread also stops several followers converging on one point and catching each other's splash.

**234 — the aim freeze.** Dragón asked twice for the follow to "fail sometimes" so he could catch a fast Pal. Implemented precisely rather than randomly: while the player is aiming at a follower, `FolowEndDistance` is raised so the action considers itself arrived and stops of its own accord. Nothing is cancelled, `Trainer` keeps being re-asserted, combat still pre-empts it at a higher priority, and looking away restores it on the next tick. Detection is arithmetic over the known followers, not `find_targeted_pal` (36-74ms, unusable at this cadence). Random failure was rejected deliberately: it would be indistinguishable from a bug in a log, which is exactly what the previous five runs looked like.

**234b — hate release.** The combat assist pushed +1000 hate per damage event and **nothing ever took it back**, so after a fight every companion still had a live target, kept hunting above Logic priority, and wandered off. Dragón diagnosed the category himself: *"i also suspect it could be one of the old codes we implemented when we were tackling the combat mechanic, maybe something from before its breaking now the behavior"*. Also fixed a friendly-fire amplifier: the "enemy" is defined as whoever the player damaged, so a stray hit on a companion aimed every OTHER companion at it. The existing guard only stopped a Pal targeting itself.

**235 — truce and recall.** Checked whether the AI preset can express "ignore other companions" and it cannot: the only discovery slots are `Discover_Player/Greater/Equal/Smaller`, categories by relative SIZE with no per-actor dimension anywhere. Hate is the only per-actor lever the game exposes, so the truce runs continuously during a fight rather than at window close — the previous version cleared 12 grudges for a fight in which two Pals were already dead and two had fled. Recall drops a strayed follower's `FindMostHateTarget()` past 1800 units so it disengages before the 3000-unit bond break.

**236-238 — the personality tag.** Player-facing names, chosen by Dragón: Normal / Curious / Timid / Aloof / Grumpy / Hostile / Feral. Bonding states (Friendly ≥20%, Bonding ≥50%) override the personality and are keyed off the BAR, not the disposition string, because a won-over Pal and a rolled-friendly Pal share the same string and cannot be told apart by it. Grumpy and Hostile stay separate on his explicit call, with a reason worth preserving so nobody merges them later as a tidy-up: *"while grumpy doesnt attack inmediately, it can attack if it sees another pal of its same species attacking, so kind of like they join"*. Styling copies `Text_Name`'s real font, colour, shadow and justification rather than building an `FSlateFontInfo`, after RenderScale/RenderOpacity produced soft, washed-out text. Position was fixed from a real geometry dump rather than a third guess.

**237 — a crash suspect removed.** `EXCEPTION_ACCESS_VIOLATION reading 0x338` on a game launched immediately after closing one. `probe_screen_projection` — a sixty-third-pass research probe that by its own design "keeps retrying each tick until a valid player controller exists" — is the only place in the mod calling engine screen projection on a controller during load, and it had already thrown a live traceback that session. Its `pcall` was never protection: pcall catches Lua errors, not native access violations. It fed nothing but log lines; the labels are positioned from the HP bar's parent panel. Removed from the tick, left defined. **Not proven to be the crash**, and recorded as such — there is no dump from that launch.

**239-240 — release prep and balance.** Verification mode off. Pet/Play 30, Feed 60, peaches 250/500 (the mod's own scale — vanilla's real values are 20000/2000, confirmed from the item data), passive +2 per 1.5s, level multiplier back on, drift once again costs trust. All test keys unbound: F9 instant-bond, CTRL+K, CTRL+H, CTRL+V, and the "4" spy — a real gameplay key a shipped mod has no business claiming. **F8 (Play) kept** per Dragón. F9 reused as the personality-tag toggle (session-only; it belongs in the settings screen when that exists). Joining now grants a flat **+50000** friendship — his correction over "raise to rank 5", and the better design: a flat grant rewards every Pal that chooses to join equally, where a target gave less to a Pal that already liked you and nothing at all to one already above it.

**241 — two real bugs Dragón caught in one run.**
...
## Publishing v1.1.0 (2026-09-12)

**Workshop.** The real item is **3797816321** ("PalBonds - The Befriending Mod", first published 2026-09-07). Its uploader folder had vanished — most likely Dragón subscribed to his own item and later unsubscribed, and Steam deletes a subscribed item's folder from the same `workshop\content\1623730\` directory the Palworld Mod Uploader works in. Re-subscribing restored it, with the real `.workshop.json` and Info.json. The 8 scripts were swapped for the stable build, only `Version` bumped to 1.1.0, and Dragón uploaded it (`.workshop.json` now records `last_published_version: 1.1.0`). The real Info.json showed the UE4SS dependency string is `"UE4SSExperimentalPW"`. The IDs 3797815819 / 3797816106 are "Create a Mod" templates that never became real items. The published 1.0.0 files are backed up in `save-backups/`.

**Nexus.** `release/PalBonds-v1.1.0.zip` reuses the old drop-in layout (`PalBonds/enabled.txt`, `README.txt`, `LICENSE`, `Scripts/`), with scripts byte-identical to the Workshop 1.1.0 upload. `release/README.txt` was corrected: its manual-install path was the Workshop UE4SS folder, which is wrong for Nexus UE4SS users (`Pal/Binaries/Win64/ue4ss/Mods/`), and it now covers food rarity, alphas and raid bosses, and the official-servers warning. `release/nexus-description.txt` keeps Dragón's wording and swaps Steam `[h2]` for `[size=5][b]` (Nexus BBCode wiki returned 403, so heading support was not confirmed), the Requirements text for a link to UE4SS Experimental on Nexus (mods/2237), and adds an install section.

**Git hygiene before pushing publicly.** Full-history audit: no passwords, tokens or keys (the "token"/"sk-" hits were ordinary words). The Steam ID had entered the not-yet-pushed stable commit through the save-backup notes in CLAUDE.md; it was removed and the commit amended before the push, so it was never published. All 40 commits carry Dragón's personal email as author — already public; rewriting history was offered, not done.

**Multiplayer, analysed not built.** See CLAUDE.md "Pending": first-player lookups everywhere, per-Pal bonding with no owner, local input versus server-side effects, and no UE4SS networking. Official servers do not allow mods.
