# PalBonds — hook-points.md historial archivado (Ninth pass a Hundred-and-eightieth pass)

Mismo criterio que `CLAUDE-archive.md` (ver esa nota para el porqué). Este archivo contiene el log técnico completo, línea por línea, de las pasadas "Ninth" a "Hundred-and-eightieth" (aprox. 2026-09-01 a 2026-09-05, antes de la investigación de balance) — movido tal cual desde `hook-points.md`, nada resumido ni borrado.

**No hace falta leer este archivo para continuar el trabajo.** `hook-points.md` (el archivo activo) mantiene: las 6 secciones de referencia permanente (respuestas a las preguntas de investigación de `DESIGN.md` §8) y las últimas ~20 pasadas (Hundred-and-eighty-first en adelante), que cubren todo el trabajo activo actual. Consultar este archivo solo para el detalle exacto de una pasada vieja específica.
...
Dragón confirmed live: friendly-labeled Pals now genuinely act friendly, and gave a standing note for this project going forward — don't use fallbacks that hide real errors/unknowns behind a pleasant-looking value while still in active development; showing the true state (even an ugly "unknown"/"fail") matters more than looking polished right now.

## Hundred-and-sixty-second pass (2026-09-04): personality rebalance + a new real-preset exclusion for NotInterested

Two requested changes, both straightforward edits to already-existing mechanisms — no new risk category.

**Rebalance:** `PERSONALITY_TIERS` — `warlike_anyway` 10→5, `warlike_without_player` 10→5, `friendly` 20→30 (still sums to 100: normal 35 / friendly 30 / escape 10 / notinterested 10 / warlike 5 / warlike_anyway 5 / warlike_without_player 5). Also fixed the `Init()` announcement log line, which still said the old percentages.

**New exclusion:** Dragón asked whether Pals whose REAL current preset is already NotInterested (several human NPCs use this) could be left alone entirely, the same way VillageNPC/Kill_All/Boss already are — while still keeping "notinterested" available as a roll OUTCOME for any other Pal. Confirmed yes, and implemented exactly that way: added `["BP_AIResponsePreset_NotInterested_C"] = true` to `EXCLUDED_FROM_ROLLING`. This checks the Pal's real, current preset class name (the same field already resolved for every Pal via `GetPresetClassName`) BEFORE the weighted roll ever runs — completely independent of the roll table itself, which still lists "notinterested" as one of its 7 possible outcomes for any Pal whose real preset is something else. No conflict between the two: one gate decides whether a roll happens at all (keyed by the Pal's real, pre-existing preset), the other is the roll's own result.

Verified with `luaparse`, deployed to both destinations. No new test needed beyond normal play — the exclusion re-uses the exact same code path already confirmed working for VillageNPC/Kill_All/Boss.

## Hundred-and-sixty-third pass (2026-09-04): a real, separate, much lower-risk base-membership field found — CTRL+H repurposed

Dragón made a sharp real-world observation that reframed the whole question: a Pal can sit in a real base with NO job assigned at all, and even human NPCs can belong to a base. That strongly suggests "is this Pal a member of my base" is a separate, simpler concept from "does this Pal have a specific job" (the `WorkAssignId`/`UPalWorkAssign` object this project just backed away from as too risky).

**Confirmed real and separate in `Pal.hpp`:** a plain `FGuid BaseCampId` field lives on `UPalIndividualCharacterParameter` — the exact object `get_individual_parameter(pal)` already returns for every single interaction in this file — with its own real update delegate (`OnUpdateBaseCampIdDelegate`), completely unrelated to `WorkAssignId` (which lives on a different class, `UPalCharacterParameterComponent`, confirmed by checking each field's actual enclosing `class` declaration line, not just proximity in the file).

**Why this is meaningfully safer than the WorkAssign idea:** no new object gets constructed, no existing Pal's specific job gets stolen or collided with. The plan copies a REAL, currently-valid base ID — read directly off some other real Pal already confirmed to be a genuine member of a real base — onto the target's own `BaseCampId` field. Same "reuse a real live value, never fabricate one" discipline already proven safe throughout this file (the same pattern used for `FPalItemSlotId`'s `ContainerId` reuse). It's still a genuinely new field being written for the first time in this project, so it's isolated on its own key with heavy logging, same as every other new-write experiment here.

**Implemented:** CTRL+H repurposed (its old job — proving the direct `SelectedFeedingItem` call unsafe — is done and confirmed). `find_donor_base_camp_id()` scans live Pals for one with a real, non-zero `BaseCampId` (same all-zero-GUID check already used in `Capture.IsAlreadyOwned`). `do_test_base_camp_membership()` targets whatever Pal you're looking at, logs its `BaseCampId` before, copies the donor's real value onto it, and logs after. Verified with `luaparse`, deployed to both destinations.

**Test plan for Dragón:** make sure at least one real Pal is genuinely placed in one of your real bases nearby (to act as the donor). Aim CTRL+H at any OTHER Pal (a wild one ideally, to test the real target case) and press once. Then immediately press "4" on that SAME Pal — does the Worker Menu open now (the one with Pet/Feed/ShowStatus/AddToParty, same as it does for a real base worker), instead of the player wheel? That's the real test of whether base membership alone (no job needed) is enough to satisfy the menu's eligibility check.

## Hundred-and-sixty-fourth pass (2026-09-05, CORRECTED below — do not treat as a confirmed dead end): the live widgets Dragón saw were a red herring from an earlier, unrelated real interaction, but the BaseCampId question itself is still open

Dragón ran the test and reported "don't think it worked," plus a raw Live View search dump for "worker" showing several LIVE instances: a `PalHUDDispatchParameter_WorkerRadialMenu`, a `WBP_WorkerRadialMenu_C`, a `WBP_WorkerRadialMenu_Overlay_C`, and five `WBP_WorkerRadialMenuContent_C` (the individual menu buttons) — which looked at first glance like evidence the real Worker Menu had opened at some point in that session. Correlating this against `palbonds-live.log` line-by-line resolved it cleanly, with timestamps.

**What actually happened, in order:**
1. **00:11:16–00:11:17** — Dragón pressed "4" on `BP_ChickenPal_C_2147428271`. `[WORKASSIGN-DIAG]` (logged in the same second) confirms this ChickenPal already had a REAL, non-empty `WorkAssignId`/`GetWorkAssign()` — i.e. it was already a genuine assigned base worker, nothing to do with this experiment. The Worker Menu opened legitimately for it (`Construct`→`OnSetup`→`CreateContent`×5→`SetupContents`→`OnSelectedEvent(index=5)`→`Destruct`/`OnClosed`), which is exactly what created the live widget instances Live View found later — they were never destroyed, just closed (same "widgets survive close, only hidden" behavior already documented in earlier passes).
2. **00:12:06** — CTRL+H on `BP_SheepBall_C` (a genuinely wild Pal, confirmed `already owned=false`, `BaseCampId is-zero=true` before). The write succeeded (`is-zero=false` after). Pressing "4" immediately after opened the NORMAL player wheel (`[RADIAL-WATCH] OpenSetup/OpenMenu/CreatePlayerActionMenu/...` — the same Otomo-wheel path this project already redirects via `SpawnedOtomo`/`TryGetSpawnedOtomo`), not the Worker Menu. Feed went through this project's own `[WILD-ACTION]` approximation (F10 HumanFeeding), same as every other wild-Pal Feed today.
3. **00:12:22** — CTRL+H again, this time re-targeting the SAME `BP_ChickenPal_C_2147428271` from step 1 (`BaseCampId is-zero=false` even BEFORE the write, confirming it already had a real base ID from being a genuine worker). No "4" press followed this one in the log.
4. **00:12:30** — CTRL+H pressed with no Pal in range — no-op, logged correctly.

**First pass at a conclusion (WRONG, corrected immediately below by Dragón's own pushback):** `WORKER-WATCH` never fired near either BaseCampId-write attempt, so this got written up as "BaseCampId alone is confirmed insufficient — third dead end." Dragón directly challenged this ("are you sure they're actual dead ends?") and was right to.

**The real finding, from re-reading `[MENU-WATCH]` (the native `StartTriggerInteract`/`TargetInteractiveObject` hook that already existed from many passes ago):** in this ENTIRE session, there was exactly ONE real native aim-lock event (`ActionType=4` with an actual `TargetInteractiveObject`) — and it's the ChickenPal's own legitimate interaction at 00:11:16, *before* either CTRL+H test. Neither BaseCampId attempt ever produced that native lock. Worse: after the second attempt (00:12:22, re-targeting the same already-real-worker ChickenPal, which tells us nothing new anyway), Dragón never pressed "4" again at all — there is no menu data whatsoever for that second attempt.

**This means the one test that mattered — the genuinely wild Sheepball at 00:12:06 — never actually reached the question it was meant to answer.** The game's own aim-detection never registered "you're looking at this specific Pal's interact sphere," so "4" fell back to the generic player/Otomo wheel for a reason already documented since passes 76-77 (no native lock → default wheel) — completely independent of whether BaseCampId would have been accepted. Declaring this a "confirmed dead end" was premature: the test's own precondition (a real native aim-lock on the target) was never verified before concluding anything about what happens once that precondition is met.

**Genuinely open, with at least three untested hypotheses:** (1) aim precision — this project's own `find_targeted_pal` tolerance (500 units / 25°) is looser than whatever cone/range the native interactable sphere actually requires; (2) Dragón's idea — the Worker Menu path might require the player to be within an actual active base's radius, untested; (3) wild Pals' interactable spheres might simply never be built to offer `ActionType=4` at all — a fixed, load-time property of the actor/sphere, not a live data check, in which case no field write (BaseCampId, WorkAssignId, anything) would ever matter.

**No new code needed for the real next test** — `[MENU-WATCH]` already logs exactly what's needed. Step 1 (baseline, no BaseCampId write at all): aim as precisely and closely as possible at a wild Pal and press "4" — check whether `StartTriggerInteract ActionType=4` ever fires with that Pal as the `TargetInteractiveObject`. If it never does even at baseline, that rules out BaseCampId (and WorkAssignId) entirely as the lever, since the gate would be structural, not field-based. Step 2, only if step 1 succeeds: retry with the CTRL+H BaseCampId copy and see if the resulting menu changes. CTRL+H's code stays as-is, no changes.

## Hundred-and-sixty-fifth pass (2026-09-05): baseline test hypothesis (1) confirmed clean — 9/9 wild Pals never got the native aim-lock; ownership-copy declined as too risky; pivoted to a Ghidra-driven direct dispatch call

Dragón ran the baseline test from the prior pass's plan. Reading the fresh session log: across 9 separate real "4" presses, `[RADIAL-REDIRECT-FIELD]` confirmed every single target was genuinely wild (`wild BP_ChickenPal_C_2147454743`, `wild BP_SheepBall_C_2147454835`) — and `StartTriggerInteract ActionType=4` with a real `TargetInteractiveObject` **never fired once** across all 9. On the 10th press, Dragón happened to aim at a *different* Pal — one already carrying a real `WorkAssignId` (a genuine base worker) — and the native lock fired immediately, full Worker Menu sequence, no issue. Clean, consistent: hypothesis (1) (aim precision) and (3) (structural, ownership-tied gate) both get support; (2) (base radius) stays entangled with ownership in this data (the one success was both owned and near a base).

To disambiguate (2) from (3) would require aiming at one of Dragón's own owned Pals far from any base — but that means copying/faking real `OwnerPlayerUId`-adjacent ownership data onto a wild Pal to test the inverse, which Dragón declined outright: **"that one has the risk of overriding a base pal isnt it? or corrupting its data since we would be copying it."** Correct call — same risk category as the Continuation 82/pass-82 incident (a copied real value colliding with another Pal's real identity). This specific disambiguation is shelved, not pursued.

**Dragón pushed back on treating the current gesture+Happy approximation as good enough** ("if the game has a system of feeding through an inventory and picking the item, is it enough for you that we simply don't use it? — because I feel like it's a sloppy job... what else are we not seeing?"). Re-opened Ghidra (the same analyzed project from passes 133-134, `_tools/ghidra/project/PalworldProject`) to look specifically at `SelectedFeedingItem` (`FUN_1430ed7e0`) — the one function already confirmed to have zero ownership check in its own body — to check a real, previously-flagged-but-unchased hypothesis: is the Crash #5 call shape simply wrong (wrong argument count/type), not a fundamental wall?

**First read of the decompile was promising but NOT reliable on its own** — Ghidra's own signature analysis for this function reported "calling convention: unknown" / "parameter count: 0," an explicit admission its automatic recovery failed here, so a single-visible-parameter pseudocode view couldn't be trusted at face value (it directly conflicts with the header-dump-confirmed, already-hooked-and-observed-working real signature `SelectedFeedingItem(FPalItemSlotId, int64)`). Rather than act on an unverified read, a second script (`check_call_site_registers.py`) disassembled the RAW instructions at the actual call site inside `FUN_1430ec9e0` (identified via the embedded delegate-binding string as `APalMonsterCharacter::OnSelectedOrderWorkerRadialMenu` — the Worker Menu's real selection-handler, already used elsewhere in this project as an `OnClose:Bind()` delegate target) right before it calls `FUN_1430ed7e0`. Register-level, ground-truth confirmation: only `MOV RCX,RDI` (loading "this") appears immediately before the `CALL` — RDX/R8/R9 are untouched for this specific call. **This specific call site genuinely only passes one argument (the Pal itself).**

Cross-referencing this against `FUN_1430ec9e0`'s full decompile resolved the real shape of the mechanism: `OnSelectedOrderWorkerRadialMenu` reads a `resultType` byte off its passed parameter (`EPalWorkerRadialMenuResult` — confirmed here to independently match the already-known live index mapping from pass 56, Cancel=0/Feed=1/ShowDetail=2/MoveToBox=3/MoveToOtomo=4/Pet=5, since the decompiled branches for `cVar3=='\x01'` and `cVar3=='\x05'` call exactly SelectedFeedingItem and a separate Pet function respectively) and dispatches accordingly — with **no ownership check anywhere in its own top-level logic either**, only a type-check on the parameter object. The real two-step mechanism: pressing Feed on the Worker Menu calls `OnSelectedOrderWorkerRadialMenu(resultType=Feed)` → that calls `SelectedFeedingItem(self)` with ONLY the Pal, which just builds and shows a real item-picker popup (no item chosen yet) → only once the player picks something does the popup's own dynamically-bound delegate fire the SEPARATE, already-hooked, already-hardware-observed-real `SelectedFeedingItem(ItemSlotId, Num)` UFunction. Crash #5 tried to jump straight to that second, "confirm" call, cold, with a fabricated `ItemSlotId` and no popup ever having existed — a plausible, well-evidenced explanation for the crash that doesn't require declaring the whole mechanism unreachable.

**New experiment implemented (`Interaction.lua`, CTRL+H repurposed a third time — the BaseCampId code stays intact, just unbound):** `do_test_direct_feed_dispatch()` finds a live `PalHUDDispatchParameter_WorkerRadialMenu` instance already in memory (via `FindAllOf`, reused rather than constructed — needs at least one real Worker Menu opened once this session first, e.g. on an actual base worker), sets its `resultType` to `1` (Feed) and its `IndividualHandle` to the targeted (possibly wild) Pal's real handle, then calls `pal:OnSelectedOrderWorkerRadialMenu(parameter)` directly — going through Unreal's own reflected UFunction dispatch (proper calling-convention marshaling handled by the engine itself, not us), never a raw/reconstructed native call. This should, if it works, make the REAL native item-select popup appear on screen for a wild Pal, with zero risk of the specific mistake that caused Crash #5.

**Honest residual risk, stated plainly to Dragón before testing:** this is the first time this project calls `OnSelectedOrderWorkerRadialMenu` itself rather than only binding it as a delegate target for the game to invoke; `SelectedFeedingItem`'s own internal resolution of what HUD/UI service to attach its popup to (a short, not-fully-traced call chain in its decompile) remains an unknown. Per Dragón's explicit instruction ("don't be afraid of the crashes as long as we can recover from them... the reward is high"), proceeding anyway, heavily logged (`[DIRECT-FEED-TEST]`, before/after every step, matching the project's crash-diagnosis discipline), one Pal at a time.

Verified with `luaparse`, deployed to the live game Mods folder. **Test plan:** press "4" on any actual real base worker once this session first (so a `PalHUDDispatchParameter_WorkerRadialMenu` instance exists in memory to reuse), then aim CTRL+H at a wild Pal and press once. Watch for whether a real item-select popup actually appears on screen, and report immediately if anything looks wrong (same alert-and-stop discipline as every prior risky test) — then check the log for `[DIRECT-FEED-TEST]` lines and, if the popup did appear and an item was picked, `[FOOD-DIAG]`/`[SLOT-USE-DIAG]` for whether real consumption followed.

## Hundred-and-sixty-sixth pass (2026-09-05): the retest went silent — a real logging bug, not a real result

Dragón retested (a real Worker Menu opened twice on an actual base worker with a real Feed selection confirmed in the log, so a live `PalHUDDispatchParameter_WorkerRadialMenu` genuinely existed in memory) and reported "didn't see any different." Reading the log: both CTRL+H attempts logged the `[DIRECT-FEED-TEST] targeting ...` line and then **nothing else at all** — not even the "no live instance found" branch, which should have logged unconditionally if `find_any_worker_menu_parameter()` had simply returned nil.

**Root cause: this project's own `safe_call()` swallows errors with zero log output by design** — correct for routine, already-proven code, wrong for a brand-new technique. `find_any_worker_menu_parameter()`'s `FindAllOf("PalHUDDispatchParameter_WorkerRadialMenu")` call was the first-ever direct lookup of this class in this project (every prior touch of it went through a hook `Context`, never `FindAllOf`) — and it was never wrapped in its own pcall, so any error there (or anywhere else in the function) propagated up and was silently eaten by the keybind handler's `safe_call(do_test_direct_feed_dispatch)`. This is exactly the kind of self-inflicted silence this project has hit before with genuinely new code paths — the fix is always the same: never trust silent-failure wrappers around code that's never been proven live yet.

**Fixed:** the `FindAllOf` call and the whole lookup loop are now wrapped in their own pcall with explicit logging (reports the raw error if `FindAllOf` itself throws, or how many instances were found/valid otherwise), and the keybind handler now uses a logging `pcall` instead of the silent `safe_call`, so any remaining uncaught error anywhere in `do_test_direct_feed_dispatch` will actually show up in the log next time, instead of dead silence.

**This means the underlying idea (call `OnSelectedOrderWorkerRadialMenu` directly) still hasn't actually been tested yet** — the retest never got past the lookup step, for reasons that had nothing to do with whether the Ghidra-derived plan is correct. Verified with `luaparse`, deployed. **Test plan, same as before:** press "4" on a real base worker once this session, then aim CTRL+H at a wild Pal and press once — this time the log should show real diagnostic detail no matter what happens.

## Hundred-and-sixty-seventh pass (2026-09-05): the new logging worked immediately — a simple ordering bug, now fixed, still nothing tested yet

The retest confirmed the logging fix worked exactly as intended: instead of silence, the log now showed a clean, actionable Lua error — `attempt to call a nil value (global 'hook_describe')`, at the exact line that called it. Real progress too: `FindAllOf` found the live parameter instance correctly ("FindAllOf found 1 instance(s) total, using the first valid one"), so that half of the lookup logic works.

**Root cause: a simple forward-reference bug, nothing to do with the experiment's design.** `hook_describe` is a `local function` declared later in this same file. Lua locals aren't hoisted — code declared earlier that references a same-named local declared later just resolves to a global instead (nil here, since it's never actually global), which is exactly the error. `do_test_direct_feed_dispatch` sits well before `hook_describe`'s declaration point.

**Fixed** by not depending on it at all — inlined the same safe pattern (`pcall(function() return parameter:GetFullName() end)`, falling back to `tostring`) directly, avoiding the ordering dependency instead of moving code around and risking touching something else.

**Still true: the actual experiment (calling `OnSelectedOrderWorkerRadialMenu` on a wild Pal) has not been reached by either retest yet.** Verified with `luaparse`, deployed. Test plan unchanged: press "4" on a real base worker once this session, then CTRL+H on a wild Pal.

## Hundred-and-sixty-eighth pass (2026-09-05): CONFIRMED — real, native, item-picker-based Feed works on a wild Pal for the first time in this project's history

Dragón ran the full test and confirmed a real success, twice: aiming CTRL+H at a wild Sheepball, a genuine item-select popup appeared, he picked Berries, and the Pal ate. The log confirms this unambiguously and completely, with no approximation involved: `[FOOD-DIAG] real SelectedFeedingItem fired — pal=BP_SheepBall_C ... Num=1` followed by `[SLOT-USE-DIAG] real PalItemSlot:RequestUseToCharacter fired — ... ItemId.StaticId=Berries ... UseNum=1` — the exact same real native consumption chain already confirmed for the player's own Otomo, now firing for a genuinely wild, uncaptured Pal. This happened twice in the same session, both clean.

**The apparent "wrong Pal got fed" was not a routing bug — both CTRL+H presses targeted the exact same actor.** Cross-checking `[DIRECT-FEED-TEST] targeting ...` against the actor IDs: both successful CTRL+H presses resolved to the SAME Sheepball (`BP_SheepBall_C_2147453268`) — Dragón's own aim landed on the same one twice, not two different ones as he believed. The Sheepball fed with the "no inventory" animation in between was a genuinely different, never-CTRL+H'd actor (`BP_SheepBall_C_2147453281`), fed entirely through the existing approximation path (confirmed via its own separate `[RADIAL-REDIRECT-FIELD]`/`[WILD-ACTION]` lines) — two visually-similar Sheepballs standing close together, not a cross-wiring bug.

**CORRECTED — Dragón directly refuted the "CTRL+H alone" reading, and he's right.** The first write-up of this pass leaned on `InputSpy`'s key log (no "FOUR" keypress found in either 3-second window) to conclude CTRL+H alone triggered the real feed both times. Dragón corrected this immediately and firmly: he pressed "4" both times, the real Otomo radial menu genuinely opened, and that's confirmed and detailed step-by-step in his own account — not something the log gets to overrule. `InputSpy`'s ~150-keybind mass listener apparently missed at least one real keypress this session — a real, newly-discovered limitation of that specific diagnostic tool, not evidence about what actually happened in the game. The real, confirmed recipe is: **CTRL+H, then "4" → pick Feed on the (existing, substituted) Otomo wheel** — both steps genuinely happened, in that order, both times.

**Revised working theory, and Dragón's own theory — which is sharper and more likely correct.** `SelectedFeedingItem`'s "open" call (triggered by CTRL+H's direct `OnSelectedOrderWorkerRadialMenu` invocation) doesn't necessarily present anything on screen by itself — per its own decompile, it writes state onto a shared HUD/UI-service object derived from the target Pal (`pwVar8`, populated via `FUN_142f6b290(param_1, ...)`). Dragón proposed directly: **"i think the ctrl + h made the wild pal eligible as an owned pal, which was the needed confirmation from the otomo radial menu to work."** This lines up precisely with the exact wall this project has been stuck on since the hundred-and-thirty-fourth pass: the Otomo widget's real `SelectedFeed` starts with two gate calls, the second (`FUN_142c8add0`) doing a raw, un-hookable C++ vtable call whose best-evidenced job is resolving "is this actor a valid target for me" — every previous attempt to satisfy that gate (overwriting `TryGetSpawnedOtomo`'s return value, writing the widget's own cached `SpawnedOtomo` field) failed because neither is what that vtable call actually reads. If CTRL+H's `OnSelectedOrderWorkerRadialMenu`/`SelectedFeedingItem` call registers the target Pal somewhere that THIS SAME vtable resolution reads — plausible, since it's a shared HUD/UI-service style object, exactly the kind of thing a generic "current valid target" lookup would consult — then CTRL+H doesn't need to open a visible popup by itself at all; its real job is making the wild Pal satisfy that one specific, previously-unreachable gate, for the immediately following "4" → Feed to then complete successfully through the pipeline that already exists and has never worked before now. This would be the actual, long-sought answer to the hundred-and-thirty-fourth pass's mystery vtable call — not proven with certainty yet, but the single most coherent explanation available, and it's Dragón's own read of the mechanism, not a guess made from the log alone.

**Dragón's other key question, directly addressed: does this permanently require having fed a real base worker first?** No — that was this pass's own cautious bootstrap choice, not a real requirement of the mechanism. `find_any_worker_menu_parameter()` only reused an already-existing live `PalHUDDispatchParameter_WorkerRadialMenu` (which only existed in memory because a real Chikipi worker had been fed earlier in the same test session) because reusing a real value is this project's established lower-risk default — not because `OnSelectedOrderWorkerRadialMenu`'s Feed branch structurally needs one that came from a real worker interaction. Per the Ghidra trace, that branch only reads `resultType` off the passed parameter. **Fixed this pass:** added `construct_worker_menu_parameter()`, using the exact same `StaticFindObject` + `StaticConstructObject` technique already proven safe in this project (Indicator.lua's trust bar and personality-label widgets) — just for a plain dispatch-parameter object this time, a smaller step than constructing a UMG widget. `do_test_direct_feed_dispatch` now tries reuse first (free, zero construction risk, when available) and falls back to fresh construction only when nothing already exists — so this should now work standalone on a brand-new save with zero real base workers ever touched. Not yet confirmed live — this construction path has never been tested in the actual game.

**Trust/friendship credit deliberately NOT wired in yet, per Dragón's explicit instruction** ("don't focus on the friendship gain for now, lets focus on polishing this and seeing where it could fail") — confirmed via log that zero `AddFriendShip` calls fired this whole session, so this real-feed path currently grants no trust progress at all. Left as a known, deliberate gap for a later pass.

**Confirmed real recipe (per Dragón's direct, corrected account, not the flawed log-only read above): CTRL+H, then "4" → pick Feed on the substituted Otomo wheel — both steps are real and required, in that order.** CTRL+H alone shows nothing on screen; it's the following "4" press, going through the existing Otomo-wheel substitution, that completes the real feed. Test plan going forward should always include both steps together, never CTRL+H in isolation.

Verified with `luaparse`, deployed. **Next real test:** confirm the new fresh-construction fallback (`construct_worker_menu_parameter`) works on a Pal where no real Worker Menu has been opened this session at all (e.g., very first thing after loading a save, before feeding any real base worker) — the log should show either "FindAllOf found... instance(s)" (reuse) or "constructed a fresh WorkerRadialMenu parameter from scratch" (the new fallback), and the same CTRL+H→"4"→Feed recipe should still work either way.

## Hundred-and-sixty-ninth pass (2026-09-05): directly testing Dragón's "made it eligible as owned" theory, and a dedicated "4" spy per his explicit request

Two additions, both diagnostic-only, requested directly by Dragón.

**Testing the ownership theory directly, not just discussing it.** `Capture.IsAlreadyOwned` reads the real, save-backed `SaveParameter.OwnerPlayerUId` GUID — the actual mechanism his theory proposes CTRL+H might flip. Added `[OWNERSHIP-CHECK]` logging (raw GUID components, not just the boolean) at three points: right before the `OnSelectedOrderWorkerRadialMenu` call, right after it returns, and — the most informative point — right when the real `SelectedFeedingItem` consumption itself fires (inside the existing, already-proven `[FOOD-DIAG]` watch hook). If his theory is exactly right, `OwnerPlayerUId` should show a real, non-zero GUID at one of these points where it showed all-zero before; if it stays all-zero throughout even as the real feed succeeds, that points instead toward a narrower, Feed-specific mechanism (this pass's own "pending request" framing) rather than a genuine ownership flip. Purely read-only, zero risk.

**Dedicated, standalone "4" keypress spy, per Dragón's direct instruction** ("put a spy everytime i press 4... just when i press '4' in the keyboard"), separate from `InputSpy.lua`'s ~150-key mass listener that just missed a real "4" press this same session. A single `RegisterKeyBind(Key.FOUR, ...)` on its own, logging only `[FOUR-KEY-SPY] "4" pressed` — nothing about menus, nothing about success/failure, exactly as asked. Kept independent in case registering ~150 keys in one batch is itself what makes `InputSpy` occasionally drop one; this key has nothing else competing with it.

Verified with `luaparse`, deployed. **Test plan:** repeat the CTRL+H→"4"→Feed recipe on a wild Pal and check the log for `[OWNERSHIP-CHECK]` at all three points, plus confirm `[FOUR-KEY-SPY]` reliably catches every real "4" press this time.

## Hundred-and-seventieth pass (2026-09-05): real, reproduced hang found — rebinding a reused parameter to a DIFFERENT Pal wedges the CTRL+H keybind itself; ownership theory tested directly, came back negative on that specific field

Dragón ran a real multi-Pal test session and asked to read the log blind before hearing his account, then separately gave a full account and directly, correctly challenged the "reuse breaks it" framing from the retest before this one: the earlier successful session ALSO reused a real object twice, so why would reuse now be unsafe?

**Ownership theory, tested directly, came back negative on that specific mechanism.** `[OWNERSHIP-CHECK]` logged `IsAlreadyOwned=false` / `OwnerPlayerUId={A=0,B=0,C=0,D=0}` at all three checkpoints (before the call, right after, and at real consumption) for the one successful attempt this session. The real save-backed ownership GUID never changed. Doesn't rule out something ownership-*adjacent* being true via a different field, but the literal mechanism Dragón proposed didn't show up here.

**The real bug, found from the log alone before Dragón's account confirmed it:** two CTRL+H attempts this session. The first (fresh-constructed parameter, wild Sheepball) completed cleanly — logged "call returned — result=ok" immediately, real feed followed. The second (reused that SAME already-used object, but rebound to a DIFFERENT Pal — a wild Chickenpal) never logged "call returned" at all — the only such call in this project's entire history to not complete. Worse: **there is no "CTRL+H pressed" log line anywhere in the rest of the session after that point** — despite Dragón confirming he pressed it at least twice more (on a Daedream, then a Gumoss). The call didn't just fail to finish; it permanently wedged the CTRL+H keybind itself for the rest of the session, in the same silent, non-crashing shape as Crash #5, just narrower in blast radius (that one wedged the whole input layer; this one wedged one specific key's async callback).

**Resolving Dragón's exact challenge — why did the earlier session's reuse work, then?** Re-checked that session's own actor IDs: both of its CTRL+H presses landed on the *exact same* Sheepball both times (already established in the hundred-and-sixty-eighth pass's write-up) — the object was reused for the SAME Pal it was already bound to, never rebound to a different one. This session's second attempt was the first time an already-used object got rebound to a genuinely different Pal, and that specific action is what appears to trigger the hang. Not "reuse is unsafe" in general — "changing which Pal a used dispatch parameter points to" is.

**This also explains every remaining detail of Dragón's account cleanly:** the Lamball got fed twice (once via the pre-existing, CTRL+H-unrelated `[WILD-ACTION]` approximation that "4"+Feed always triggers regardless, once via the real inventory from the successful CTRL+H call — two independent systems, not a bug); the Chikipi's popup opened but showed nothing and needed Escape (a partially-constructed UI left over from the hang); and the Daedream/Gumoss attempts both fell back to the plain animation with zero real popup, because CTRL+H itself had already stopped firing by then.

**Fixed:** `do_test_direct_feed_dispatch` no longer calls `find_any_worker_menu_parameter()` (reuse) at all — always constructs a brand-new parameter object per attempt now, unconditionally. The reuse helper is left in the file, unused, with a note explaining why. Also switched the "4" spy from `RegisterKeyBind` to `RegisterKeyBindAsync` — the same mechanism CTRL+H/CTRL+K/Play use, none of which have ever missed a press in this project's history, unlike the plain version both spies used.

**Practical note for Dragón, worth restating from Crash #5's own lesson:** once a keybind stops responding mid-session like this, the game can keep looking completely alive while that specific piece of Lua state stays wedged — a fresh game restart before the next test is the safest way to make sure nothing from the hang carries over.

Verified with `luaparse`, deployed. **Test plan:** repeat CTRL+H→"4"→Feed across several different wild Pals in one session (always fresh construction now) and confirm CTRL+H keeps responding every time, not just the first — plus check whether `[FOUR-KEY-SPY]` (now async) catches every real "4" press this time.

## Hundred-and-seventy-first pass (2026-09-05): REAL CRASH — pressing "4" too close to CTRL+H runs two real native systems concurrently on the same Pal

Dragón got a genuine crash (Fatal Error, Windows crash dump) pressing "CTRL+H, then 4" quickly. The log confirms exactly why: `CanOpenPlayerActionMenu` (the real signal that "4" was processed) appeared IN THE MIDDLE of our own CTRL+H lines, while `do_test_direct_feed_dispatch` was still executing. The real "4" substitution system (`RADIAL-REDIRECT`, a second `CanOpenPlayerActionMenu`) kept firing at nearly the same instant as our own `pal:OnSelectedOrderWorkerRadialMenu(parameter)` call — then the log simply stops and the crash dialog appeared. Two real systems constructing native UI on the SAME Pal at the same instant — not the call itself, which has completed cleanly many times in isolation.

**Fixed:** moved the declaration of `radialMenuActionWindowOpen` (an already-existing real flag, true for the entire duration of a real "4" menu window) much earlier in the file so `do_test_direct_feed_dispatch` can read it. Right before the risky call now, if that flag is true (a real "4" menu is active at that exact moment), the function aborts with a clear message instead of racing it.

**Honest limit of this fix:** it only covers one direction — CTRL+H running first, "4" pressed during its execution (exactly what happened here). It can't detect the reverse order (if "4" is pressed first and CTRL+H lands a few milliseconds later, before its own flag state settles). Because of that, the real standing safety rule for testing stays: never press "4" and CTRL+H close together — wait a couple of seconds between them regardless of order.

Verified with `luaparse`, deployed.

## Hundred-and-seventy-second pass (2026-09-05): RESOLVED — CTRL+H genuinely works standalone; the whole "4" confusion was a multi-second visibility delay, nothing more

Dragón followed the wait-and-don't-press-"4" instruction properly this time, and the result settles the question definitively, with both spies working and unambiguous timestamps. CTRL+H's own call to `OnSelectedOrderWorkerRadialMenu` returned "ok" in ~170ms — but the real popup didn't become clickable until roughly 3.6 seconds later (Dragón estimated "around 10 seconds," same phenomenon, different perception of the wait). The real feed then fired off a plain `LEFT_MOUSE_BUTTON` click — with zero "4"/`FOUR` activity anywhere nearby, confirmed by both `[FOUR-KEY-SPY]` (now firing reliably, async) and `InputSpy`. A "4" press 8 full seconds *later* triggered a completely separate, second feed through the old approximation system on the same Pal — an unrelated action Dragón took afterward, not part of the real mechanism at all.

**Dragón's own words nail the real explanation:** "so all this time its just that ctrl+h has a terrible delay, which made me think it activated by the 4 radial menu." Exactly right. There was never a real dependency on the Otomo/"4" system — both the "unlocks the existing Otomo pipeline" theory (this project's own) and "made the wild Pal eligible as owned" theory (Dragón's) were built on the false premise that "4" was actually required. It never was. The real mechanism is exactly as simple as the original Ghidra finding suggested from the start: `SelectedFeedingItem`'s open call has no ownership check and needs nothing else — CTRL+H alone opens the real popup, full stop. The only real gap was that this project never accounted for a multi-second UI-construction delay before assuming a call "did nothing."

**Fixed:** both of CTRL+H's own log lines (the Init() announcement and the post-call message) now explicitly say it works standalone, warn about the multi-second delay before the popup becomes visible/clickable, and warn not to press "4" while waiting (real crash risk, hundred-and-seventy-first pass). No code logic changed this pass — this was a documentation/messaging fix, the mechanism itself was already correct.

This closes out the real, long-standing question at the heart of item 7's investigation (real item-based Feed for wild Pals): **it works, standalone, via CTRL+H, confirmed twice now with clean unambiguous data.** Remaining open items for this feature: trust/friendship credit is still not wired in (Dragón's explicit "not yet"), and the crash-avoidance guard from the previous pass only covers one direction of the "4"-interference race — though since "4" is now confirmed unnecessary, the simplest and safest practical rule is just: don't press "4" near CTRL+H at all.

## Hundred-and-seventy-third pass (2026-09-05): wired the confirmed-real mechanism into the ACTUAL "4" menu, replacing the approximation for wild Feed

Dragón asked directly: "lets add it to the menu, lets see how it works with the actual radial menu instead of ctrl + h." Wired `OnSelectedOrderWorkerRadialMenu`/fresh-parameter-construction (the exact same proven call shape as `do_test_direct_feed_dispatch`) into `closeRadialMenuActionWindow` — the function that already fires the real action when the "4" menu closes with a substituted wild Pal and a decided "feed" instruction.

**New function, `do_real_wild_feed_via_worker_menu`:** targets `cachedRedirectWildPal` (the live wild Pal this exact menu window has been substituting, not a fresh re-aim — the player may not still be looking at the Pal by the time the menu closes), constructs a brand-new parameter (never reuses — hundred-and-seventieth pass's hang), writes `IndividualHandle`/`resultType`, and calls the real function. Returns `true`/`false` rather than raising, so `closeRadialMenuActionWindow`'s Feed branch can fall back to the old approximation (`do_feed`) automatically if anything about the real path fails to even get set up — Dragón is never left with silent nothing.

**This replaces the approximation for wild Feed, it doesn't run alongside it** — once the real call succeeds, `do_feed` is not also called (unlike CTRL+H, which always ran independently of whatever "4" was doing). Pet/Care via "4" is completely untouched — it already goes through the real vanilla Care action and was never part of this problem.

**Deliberately checked, even though the specific race that caused the hundred-and-seventy-first pass's crash shouldn't apply here:** by the time `closeRadialMenuActionWindow` runs, the "4" menu's own OPENING sequence (`CanOpenPlayerActionMenu`/`OpenSetup`) has already fully completed — the danger there was specifically our call racing that opening sequence, not existing during a close. Added the `radialMenuActionWindowOpen` check anyway as a defensive measure, purely as a "should never be true here, but not worth finding out" safeguard.

**Same expected behavior as before, now inside the real Feed flow instead of a separate key:** the real item-picker popup can take several seconds to actually appear/become clickable AFTER the "4" menu has already visibly closed — not a bug, already confirmed twice.

Verified with `luaparse`, deployed. **Test plan, cautious first run:** feed ONE wild Pal via the real "4" menu (aim, press "4", pick Feed, let the menu close normally) and watch for: (1) the real item-picker popup appearing a few seconds later — pick an item and confirm the animation/consumption happens; (2) the OLD approximation animation should NOT also play — this is meant to fully replace it now; (3) if anything looks wrong, report immediately, same as every other real-effect change in this file.

## Hundred-and-seventy-fourth pass (2026-09-05): self-inflicted bug — the defensive `radialMenuActionWindowOpen` guard fired every single time, blocking the real call permanently

Dragón tested and got the old approximation every time — the real popup never appeared. The log showed why immediately: `[REAL-FEED] ABORTING — the real "4" menu window is still marked open` fired on both attempts. Wrong assumption in the previous pass's own guard: `closeRadialMenuActionWindow` only sets `radialMenuActionWindowOpen = false` AFTER calling the Feed branch, not before — so the flag is ALWAYS true at the exact point this new check ran, by design, every time, with no exception. The comment even said "should not happen at close time" — it was never checked against the actual surrounding code before being added.

**Also worth noting why the guard was never actually needed here in the first place:** the hundred-and-seventy-first pass's crash came from two SEPARATE, independently-triggered key events (a standalone CTRL+H press racing a standalone "4" press) executing concurrently. This call site is different in kind — `do_real_wild_feed_via_worker_menu` runs synchronously, in a single call chain, as a direct continuation of the SAME "4" press's own close-handling logic. There's no second, independent trigger involved at all, so this specific reentrancy danger was never actually present here.

**Fixed:** removed the check entirely rather than trying to patch its condition — it wasn't protecting against anything real at this call site.

Verified with `luaparse`, deployed. **Test plan, same as the previous pass:** feed a wild Pal via the real "4" menu and confirm the real item-picker popup now actually appears (with the known few-second delay) instead of falling back every time.

## Hundred-and-seventy-fifth pass (2026-09-05): CONFIRMED WORKING — real inventory Feed for wild Pals, live in the actual "4" menu, no delay this time

Dragón tested and confirmed a full, clean success: aimed "4" at a wild Sheepball, picked Feed, the real item-picker opened, picked a Berry, real consumption/animation followed — through the actual game menu, not an isolated test key. This is the real, permanent answer to item 7 (real item-based Feed for wild Pals), now live in the default interaction flow rather than an experimental key.

**Notable and worth remembering: no multi-second delay this time**, unlike every prior CTRL+H-only test. Likely explanation, not yet confirmed: going through the real "4" flow means the game's own UI/input system is already "warmed up" (a menu was just open, closing) at the moment `OnSelectedOrderWorkerRadialMenu` fires, whereas CTRL+H calls into it completely cold, from a stopped/idle input state — plausible enough not to treat the earlier delay as a fixed property of the mechanism itself.

**Current state of item 7:** the mechanism is real, confirmed twice in two different contexts (standalone key, and now the live "4" menu), and replaces the gesture+Happy approximation for wild Feed specifically (Pet/Care untouched). Trust/friendship credit for this path is still not wired in, per Dragón's own explicit "not yet" — the real consumption and animation both work, but this specific Feed path grants no bonding progress yet. That remains the next real step whenever Dragón wants it.

## Hundred-and-seventy-sixth pass (2026-09-05): Kinship Peach investigation — confirmed zero real friendship, and CTRL+H repurposed for the first-ever write to real persisted inventory data

With the real inventory-Feed mechanism live, Dragón tested it directly for the "Kinship Peach" question this project has carried since Continuation 34: fed a wild Eikthyrdeer his one real lesser Kinship Peach through the real "4" menu, then petted it 6-7 times to trigger a real sphere-less capture.

**Real item ID confirmed:** `AffectionFruit_02` — matching one of the two candidate names found in the game's own item tables back in the eighty-third pass, now confirmed via the same live `[SLOT-USE-DIAG]` hook already used for Berries.

**Real friendship contribution from the peach: mathematically confirmed zero.** No `AddFriendShip` call fired anywhere near the moment of consumption (a clean 16-second gap). The Pal's final real point total at capture (70) decomposes exactly into 7×10 — matching the 6-7 real Pets that followed, each crediting +10 via the already-established mechanism (the real vanilla Care action firing independently on the substituted wild Pal, per the hundred-and-fifty-ninth pass). No residual, no unexplained bump — the peach's real vanilla bonus, whatever it is, does not apply through this specific consumption path for a wild Pal still mid-bonding.

**Side finding, not new but reproduced again clearly:** this project's own interaction counter only ever logged `"interaction #1"` despite 6-7 real credited Pets — the same already-documented gap where `do_pet()`'s own busy-gate discards almost every press while the real vanilla animation grants credit through a completely separate path. The point total stayed accurate (drives the correct capture-threshold behavior); only the interaction *count* undercounts for Pets going through the real substituted menu.

**Duplicating the item for further testing — Dragón's own framing adopted directly:** rather than treat this as needing the same rigor as the core PalBonds mechanics ("no other mod does this, we fought hard for it"), he pointed out plainly that item spawning/duplication is different — it's already been solved publicly, so even a failed attempt costs nothing (a known-working mod is the fallback). Investigated a real, open-source reference (`starpoem17/PalWorld_ItemConsole` on GitHub) rather than guessing — its own item-duplication feature (`item.drop`, explicitly "creates one world pickup... without consuming inventory") turned out to be backed by a full custom native C++ DLL (`native/PalItemNativeDrop/`, built via CMake, exposing a custom `PalItemNativeDrop` Lua global) — a materially different, heavier engineering category than anything this all-Lua project has done. Not the "should be easier" shortcut it looked like at first glance.

**The actually-applicable, much simpler path:** Dragón doesn't need a general item spawner — he already owns Kinship Peaches and just wants more of that specific item. `find_best_food_slot()` (already proven safe, already used for Berries) finds the real, live `PalItemSlot` holding a given item; writing its `StackCount` field is the same category of plain field write already done safely elsewhere in this file (`BaseCampId`, `IndividualHandle`, `resultType`) — just the first time touching something that's real, persisted save data rather than ephemeral UI/interaction state.

**CTRL+H repurposed again** (its previous job — proving the direct-dispatch mechanism — is confirmed and now permanently wired into the real "4" menu, so it no longer needs its own dedicated test key; old body commented out, not deleted, per this project's standing convention): `do_test_multiply_kinship_peach()` finds the real slot holding `AffectionFruit_02`, logs the real current count, and writes a conservative target count (10 — not an extreme number, since this item's real one-stack cap is unknown and this is genuinely new territory). Logs before and after the write so the result is unambiguous either way.

Verified with `luaparse`, deployed. **Test plan:** press CTRL+H once, then check the real in-game inventory — does the peach count actually show 10? Does it look/behave normal (no visual corruption, no crash, can still be selected/used normally in a real feed)? Report exactly what you see either way.

## Hundred-and-seventy-seventh pass (2026-09-05): confirmed and fixed — `RequestUseToCharacter` never decrements for a wild target, the exact same limitation already found once before via CTRL+J

Dragón bumped the peach to 10, then ran a controlled 6-feed test on a fresh wild Tanzee Ignis (Berry, Berry, Peach, Peach, Berry, Peach). Every single one read the exact same pre-feed count — Berries stuck at 110 all three times, peaches stuck at 10 all three times. Zero movement, not intermittent — a clean, total negative result.

**This isn't new — it's the same wall this project already hit and worked around once, from a different angle.** Back in the CTRL+J experiment (hundred-and-seventeenth/eighteenth passes), calling `RequestUseToCharacter` directly on a wild Pal "said yes" but never actually moved the stack, while the identical call on the player's own Otomo worked correctly both times. The real "4" menu's new Feed path ultimately calls into this same function under the hood, so it inherits the identical silent no-op for a non-owned target.

**This also gives a real, unifying explanation for the missing friendship** (both the Eikthyrdeer and this Tanzee): if the game's own logic never treats the item as genuinely consumed for a target it doesn't consider truly owned, then whatever internal logic would grant a friendship bonus for that consumption — including any special Kinship-Peach-specific bonus — never runs either. Same gate, both symptoms.

**Fixed, reusing the exact fallback already proven safe in the old CTRL+J mechanism:** added a POST-hook on `RequestUseToCharacter` (the existing pre-hook watch stays as pure logging, unchanged). A new file-level flag, `pendingWildFeedTarget`, is set by `do_real_wild_feed_via_worker_menu` right before its risky call and consumed (one-shot) by the post-hook — only acting when it's set, so a normal owned-Pal feed (which already decrements correctly on its own, confirmed via this same session's party-Pal feeds) is never touched twice. When set, the post-hook manually subtracts `UseNum` from the slot's real `StackCount` itself, logging before/after.

Verified with `luaparse`, deployed. **Test plan:** repeat a real wild Feed (any item) through the "4" menu and confirm the real inventory count now actually drops by 1 each time, with a `[MANUAL-DECREMENT]` log line showing the before/after numbers.


## Hundred-and-seventy-eighth pass (2026-09-05): real FPS testing exposed two live lag/correctness bugs — a dead investigation hook firing on every nearby object, and human NPCs getting swept into the personality roll

While running a real PresentMon FPS comparison (modded vs. modless, a side-task unrelated to mod code), Dragón broke the modded capture down by presentation mode and flagged something separate and real from the raw numbers: he described two specific things he suspected were causing the noticeable in-game lag with mods active — "the constant check of what im looking at (for wild pals), which probably triggers when i even look at a tree" and "a confirmation that runs over and over for nearby pals... I often see it in the console." Pulled the actual `palbonds-live.log` from that same session and broke it down by tag volume instead of guessing.

**Confirmed #1 — `[MENU-WATCH]`, 1,048 of 6,384 lines (16%).** This is the thirteenth-pass `PalInteractComponent:StartTriggerInteract`/`EndTriggerInteract` watch hook — real samples showed it firing with `ActionType=2/3/4` on the player's own interact component regardless of what's aimed at (trees, storage, structures — anything interactable, not just Pals), exactly matching Dragón's own description. Checked whether anything still depends on it: `lastAimedInteractTarget` (written here on every `ActionType=4`) is never read anywhere else in the file anymore — the real wild-feed mechanism (hundred-and-sixty-eighth/seventy-third passes) uses `cachedRedirectWildPal` instead, so that write is dead. The `WORKASSIGN-DIAG` block inside it ran a full `find_targeted_pal()` scan (the same ~35-50ms-per-call scan already documented as costly, `RADIAL-REDIRECT-PERF`, seventy-seventh pass) on every real "4" press, investigating a `WorkAssignId` eligibility theory that was superseded by the BaseCampId findings (hundred-and-sixty-third pass) — that investigation is over. `EndTriggerInteract`'s hook was pure logging. Confirmed fully dead, not just noisy — commented out in full (old body preserved, per this project's usual practice).

**Confirmed #2, but heavier than Dragón's own guess of a "confirmation ping."** `[DIAG-CREATE]`/`[DIAG-PANEL]`/`[DIAG-TRUST]`/`[DIAG-LABEL]` in `Indicator.lua` — ~2,437 lines combined. Real samples show this isn't a cheap check, it's full widget construction (`StaticConstructObject`, `AddChildToCanvas`, `SetPosition`, `SetSize`, resolving the real Pal actor, creating a text label) happening from scratch ~315 times in a 3-minute base session (~1.75/sec). The game recycles a shared pool of HP-gauge widgets across whichever Pals are currently visible (confirmed since the forty-fifth/forty-sixth passes) — every time one gets reused for a different Pal, this mod has to rebuild the whole trust bar + personality label on it. This is real, wanted functionality (not dead code), so it wasn't touched this pass — needs an actual design decision (throttle rebuilds, cache more aggressively by Pal ID, only update what changed instead of a full rebuild) before changing it, deferred to Dragón.

**Separately, Dragón reported something more serious found while actually playing post-grind: reaching a town, traders (human NPCs) were rolling personality tiers and one came out hostile — actual traders attacking him.** `EXCLUDED_FROM_ROLLING` (hundred-and-fifty-third/sixty-second passes) was supposed to prevent exactly this, but it only ever worked by matching the individual's REAL resolved preset class name (`VillageNPC`/`Kill_All`/`Boss`/`NotInterested`) — and preset resolution depends on the whole `AISensorComponent`/`AIResponsePreset` chain already documented elsewhere in this file as frequently failing to resolve at all. A human NPC almost certainly doesn't resolve a preset the same way a monster does, so it silently fell through the exclusion table (wrong/nil key) straight into the random roll.

Dragón also asked to check the "Pal Analyzer" reference mod (`research/unpacked/PalAnalyzer/`) for a lower-lag technique for showing Pal info, since it was never fully investigated before. Extracted its `.uasset`/`.uexp` strings (no `strings` binary available in this shell — used `grep -aoE '[ -~]{5,}'` as a substitute, same workaround as the ninety-third pass's failed attempt, successful this time) and found two genuinely new, useful things at once:

- Its design is hover-based, not scan-all-visible-Pals: `HoveredCharacter`, `IsValidHoverCreature`, `CallFunc_BreakHitResult_*`, `CallFunc_GetCameraLocation/GetCameraRotation` — it line-traces from the camera and only ever builds info for whatever's currently under the crosshair, never one widget per visible Pal simultaneously. That's structurally why it doesn't have `Indicator.lua`'s per-visible-Pal rebuild cost — a genuinely different design, not a cheaper implementation of the same one. Not adopted this pass (Indicator.lua's whole point is showing trust bars on ALL nearby Pals at once, not just one hovered target — a hover-only redesign would change what the feature does, not just how it's built — flagged for Dragón to weigh in on before ever pursuing).
- Its Blueprint graph explicitly branches on `CallFunc_IsPalMonster_ReturnValue`/`CallFunc_IsWildNPC_ReturnValue` before treating something as an actual creature (`AnalyzeDomestic_52_...` vs `AnalyzeHumans_51_...` graph nodes). Confirmed live in this game's own native header dump: `bool UPalUtility::IsPalMonster(const AActor*)` and `bool UPalUtility::IsWildNPC(const AActor*)` (`CXXHeaderDump/Pal.hpp`, class `UPalUtility` — the exact same trusted static utility class this project already calls via `get_pal_utility()` for `GetIndividualCharacterHandleByActor`/`PalCaptureSuccess`/etc.). Unlike the preset chain, this takes the raw actor directly — no `AISensorComponent`, no preset object, nothing that can silently fail to resolve the way the existing exclusion did.

**Fix, `Personality.lua`:** added `is_confirmed_pal_monster(palActor)` — calls `utility:IsPalMonster(palActor)`, returns `false` (not a monster) on ANY failure (missing utility, call error, non-true result) rather than defaulting to "let it roll." This is deliberately the same safe-failure convention as `Capture.IsAlreadyOwned` — a false exclusion just costs one Pal its personality variety; a false inclusion is a human NPC turned hostile, which is strictly worse. Wired as an unconditional gate in `GetOrInitState`, checked BEFORE the preset-name table: `if not is_confirmed_pal_monster(palActor) then rolledTier = "normal"` — catches human NPCs even when their preset never resolves at all, which is exactly the scenario that let the trader through. The existing `EXCLUDED_FROM_ROLLING` preset-name check is left in place as a secondary layer (still useful for real monster Bosses, which DO pass `IsPalMonster` but should also stay normal per Dragón's original ask).

Verified with `luaparse` on both files, deployed to the live Mods folder. **Test plan:** visit a town with traders/human NPCs again — none of them should roll any tier other than "normal" going forward, regardless of whether their preset resolves. Separately, the `[MENU-WATCH]` removal should show up as a real, measurable drop in log volume/console spam next session, though the trust-bar rebuild cost (#2 above) is untouched and still a live, unresolved lag contributor pending a design decision.


## Hundred-and-seventy-ninth pass (2026-09-05): NotInterested exclusion removed (superseded), Pal Analyzer read corrected, InputSpy finally disabled

Three follow-ups from Dragón on the previous pass's findings, all real corrections rather than confirmations.

**1. `EXCLUDED_FROM_ROLLING`'s `NotInterested` entry removed.** Dragón, directly: "that means we can include notinterested pals once more... now that you've found that separation, then we can use that instead." That entry (hundred-and-sixty-second pass) only ever existed as an indirect proxy for catching human NPCs by preset name, since several use `NotInterested` as their real default. With `is_confirmed_pal_monster` now gating `GetOrInitState` directly and unconditionally before this table is even consulted, the proxy is redundant — removed. Any actual monster whose species default happens to be NotInterested can now roll a real personality tier like any other. `VillageNPC`/`Kill_All`/`Boss` were left in place — Boss in particular is a real monster-preset category (Alpha Pals) Dragón wants excluded regardless of the human/monster question, and removing the other two wasn't asked for.

**2. The "Pal Analyzer only shows one hovered Pal" claim from the previous pass was wrong.** Dragón pushed back with a real observation: a screenshot from that mod shows several Pals with analyzer boxes open simultaneously. Re-checked the extracted strings more thoroughly instead of standing by the first read — found `CallFunc_SphereTraceMultiForObjects_OutHits`/`SphereTraceMultiForObjects` (a multi-hit trace within a radius, not a single-target line trace) plus a real `Map_Add`/`Map_Find`/`Map_Keys`/`Map_Values` (a TMap, almost certainly Pal actor → its analyzer widget) and a loop counter variable. The earlier "hover-only" read was based on incomplete evidence — it only checked for the single-hit trace functions (`BreakHitResult`, `GetCameraLocation`) and stopped there without checking for a multi-hit path that also exists in the same Blueprint. Corrected conclusion, more useful than the wrong one: the real technique is very likely that it maintains its OWN Pal→widget map, creating a widget once per Pal actor and reusing/repositioning it while that Pal stays in range, rather than piggybacking on the game's own HP-gauge widget pool the way `Indicator.lua` does. This is a concrete, evidence-backed direction for the still-open lag-source-#2 design question (trust-bar/label full rebuild on every native gauge-widget recycle) — not implemented yet, offered to Dragón as the next concrete option rather than assumed.

**3. `InputSpy.lua` finally disabled.** Its own file header (and `main.lua`'s require-line comment) already documented exactly when to turn it off: "once the radial-menu question is closed." That question has been closed since the hundred-and-seventy-third pass (real wild-Feed-via-menu confirmed working end to end). Dragón noticed it still firing on every WASD/mouse press in a fresh console excerpt and asked directly why it was still there — no good reason remained. `InputSpy.Init()` commented out in `main.lua` (require() left in place, harmless on its own). This removes a real, continuous per-movement-key log-volume cost that the previous pass's tag-volume breakdown didn't even catch, because its log lines use a `[PalBonds/InputSpy]` module tag rather than a second bracketed `[TAG]` — the grep used to rank lag sources by volume only matched `[A-Za-z0-9_-]+` bracket contents and silently missed anything shaped like `[PalBonds/InputSpy]` entirely.

Verified with `luaparse` on all three files (`Personality.lua`, `main.lua`), deployed. **Test plan:** visit a monster (not human) whose species default happens to be NotInterested, if one comes up, and confirm it can now roll a real tier; general play should show a real drop in log volume now that InputSpy is off, on top of the MENU-WATCH removal from the previous pass.


## Hundred-and-eightieth pass (2026-09-05): implemented the Pal Analyzer's own-Pal-map technique for the trust bar — reuse instead of rebuild on gauge recycle

Dragón approved trying the technique found in the previous pass's Pal Analyzer investigation: "lets try using that then, see if the lag drops more." Implemented in `Indicator.lua`.

**The change:** `install_trust_bar(gaugeWidget)` now tries `resolve_pal_actor_from_gauge` + `Personality.GetStableId` BEFORE building anything. If this exact Pal already has a live entry in `trackedBars` (keyed, from now on, by Pal ID rather than gauge-widget identity whenever that ID is known), a new `reparent_existing_bar(entry, newGaugeWidget)` moves the ALREADY-BUILT `UProgressBar` + personality-label widgets onto the new gauge's real parent panel via `UPanelWidget:RemoveChild` (confirmed real in this game's own `UMG.hpp`, same header this file already trusts for `AddChildToCanvas`) followed by `AddChildToCanvas` again — the widget OBJECTS persist, only their panel membership and position change. This skips `StaticFindObject`/`StaticConstructObject`/the label-class lookup entirely for a Pal already being tracked, which — per the previous pass's real numbers (315 full builds in 3 minutes against far fewer actually-distinct Pals near a base) — is the common case, not the exception.

Handles the case where the Pal ID isn't resolvable yet at gauge-creation time (the usual `BindFromHandle` race, already documented) by falling back to the old behavior: build fresh under a temporary gauge-widget key, exactly as before. `update_trust_bars`'s existing per-tick retry, when it successfully resolves the actor later, now also tries to resolve the Pal ID and — if this Pal doesn't already have a DIFFERENT tracked entry — promotes this entry from its temporary key to the real Pal-ID key, so a LATER gauge recycle for the same Pal can find and reuse it too. Promotions are collected during the loop and applied AFTER it, not during — `pairs()` in Lua explicitly allows nil-ing an existing key mid-traversal (already relied on above for dropping dead entries) but not inserting a brand-new one, so a same-loop write would have undefined behavior.

**Left as accepted, rare debt, not solved this pass:** if a gauge's Pal ID resolves successfully on the FIRST try (immediate, not via retry) for a Pal that ALSO has an existing but not-yet-promoted entry under a temporary key elsewhere, this pass doesn't merge them — two bars would briefly exist for the same Pal until one goes stale and gets dropped by the existing invalid-widget cleanup. Judged not worth the complexity of a full merge/destroy path for what should be a rare timing coincidence, versus the common recycle case this pass is actually targeting.

Verified with `luaparse`, deployed. **Test plan:** a busy-base session like the one that produced the 315-creates number — check whether `[DIAG-CREATE] REUSED existing bar` lines start appearing (confirms the reuse path is firing) and whether the full-construction `[DIAG-CREATE] attempting to construct...` lines drop substantially in the same kind of session, alongside a real, felt reduction in lag.
